...
def _propagate_C(
    ...
    *,
    ...
    r"""Propagate C by tau using a factored 3d×3d matrix exponential.

    The ODE is Ċ = BᵀB.  The augmented Hamiltonian H_C has B_ref^T as a
    matrix entry.  Near δ-BC boundaries B_ref = O(1/ε), making matrix_exp(H_C τ)
    numerically unstable.

    We exploit the block-lower-triangular structure of H_C:

        H_C = [[H2d, 0], [R, 0]]   where R = [0, B_ref^T]  (d × 2d)

    The matrix exponential of a block-lower-triangular matrix is:

        exp(H_C τ) = [[exp(H2d τ), 0], [∫₀^τ R exp(H2d s) ds, I]]

    so Γ₂ = ∫₀^τ B_ref^T Φ₂₂(s) ds = B_ref^T × ∫₀^τ Φ₂₂(s) ds.

    The integral ∫₀^τ Φ₂₂(s) ds is obtained from exp(H̃_C τ) where H̃_C
    replaces B_ref^T with I:

        H̃_C = [[H2d, 0], [[0, I], 0]]

    H̃_C has entries of O(max(‖σ‖, ‖β‖, 1)), independent of ‖B_ref‖.
    ...
    # Build factored H̃_C: same structure as H_C but with I replacing B_ref^T
    ...
    Gamma2_tilde = Phi_tilde[2*d:, d:2*d]   # (d, d) = ∫₀^τ Φ₂₂(s) ds
